...
const stats = [
  ...
  { title: "HITL Verified", hindi: "सत्यापित अभिलेख", value: "1,102", change: "99.1% accuracy", icon: CheckCircle2, tone: COLORS.green },
  { title: "Pending Review", hindi: "सत्यापन लंबित", value: "126", change: "18 urgent queues", icon: Clock3, tone: COLORS.saffron },
  { title: "Low Confidence", hindi: "निम्न विश्वसनीयता", value: "56", change: "Flagged for AI audit", icon: AlertTriangle, tone: COLORS.red },
];

// Mock parcel data keyed by khasra number — replace with a real
// GIS/cadastral API lookup (e.g. Bhunaksha / State GIS server).
const MOCK_PARCELS = {
  "124/2": { village: "Chandrapur", area: "1.25 Hectares", coords: [[60, 40], [180, 30], [190, 120], [70, 130]], center: [130, 80] },
  "88/1": { village: "Basti Kalan", area: "0.8 Hectares", coords: [[40, 90], [140, 60], [170, 140], [50, 160]], center: [110, 110] },
  "210": { village: "Rampur", area: "2.1 Hectares", coords: [[70, 30], [200, 50], [180, 150], [60, 130]], center: [130, 90] },
};
...
function GISLookup() {
  ...
  const handleLookup = (e) => {
    e.preventDefault();
    const match = MOCK_PARCELS[khasra.trim()];
    if (match) {
      setParcel(match);
      setNotFound(false);
    } else {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="font-bold flex items-center gap-1.5">
            <Crosshair size={16} style={{ color: COLORS.navy }} /> GIS Khasra Lookup
          </h3>
          <p className="text-xs text-slate-500">Enter a khasra number to plot its boundary on the cadastral map</p>
        </div>
      </div>
      ...
      <form onSubmit={handleLookup} className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 rounded-xl border border-slate-300 px-3 py-2 text-sm font-mono outline-none focus:ring-2 focus:ring-blue-200"
        />
        <button type="submit" className="rounded-xl px-4 py-2 text-sm font-semibold text-white shadow-sm" style={{ background: COLORS.navy }}>
          <Search size={15} />
        </button>
      </form>

      <div className="rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
        <svg viewBox="0 0 240 200" className="w-full h-52">
          {Array.from({ length: 12 }).map((_, i) => (
            <line key={"v" + i} x1={i * 20} y1={0} x2={i * 20} y2={200} stroke="#E2E8F0" strokeWidth="0.5" />
          ))}
          {Array.from({ length: 10 }).map((_, i) => (
            <line key={"h" + i} x1={0} y1={i * 20} x2={240} y2={i * 20} stroke="#E2E8F0" strokeWidth="0.5" />
          ))}

          <rect x="10" y="10" width="60" height="50" fill="#EFEAD9" stroke="#D9D2BE" strokeWidth="1" />
          <rect x="170" y="140" width="60" height="50" fill="#EFEAD9" stroke="#D9D2BE" strokeWidth="1" />

          {parcel && (
            <>
              <polygon
                points={parcel.coords.map((p) => p.join(",")).join(" ")}
                fill={COLORS.green + "33"}
                stroke={COLORS.green}
                strokeWidth="2.5"
              />
              <circle cx={parcel.center[0]} cy={parcel.center[1]} r="4" fill={COLORS.navy} />
              <text x={parcel.center[0] + 8} y={parcel.center[1] + 4} fontSize="9" fontFamily="monospace" fill={COLORS.ink}>
                {khasra}
              </text>
            </>
          )}
      ...
      {parcel && (
        <div className="mt-3 flex items-center justify-between text-xs bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-2">
          <span className="flex items-center gap-1.5 text-emerald-800 font-medium"><MapPin size={12} /> {parcel.village}</span>
          <span className="font-mono text-emerald-700">{parcel.area}</span>
        </div>
      )}
      {notFound && (
        <p className="mt-3 text-xs text-red-600 flex items-center gap-1.5"><AlertTriangle size={12} /> No parcel found for that khasra number. Try 124/2, 88/1, or 210 (demo data).</p>
      )}
      {!parcel && !notFound && (
        <p className="mt-3 text-xs text-slate-400">Try a demo khasra: 124/2, 88/1, or 210.</p>
      )}
    </div>
  );
}
...
const INTEROP = [
  { icon: Banknote, tone: COLORS.navy, title: "Banks & Lending", desc: "Verified titles serve as collateral proof for instant agricultural or property loans — no physical document submission." },
  { icon: Wheat, tone: COLORS.green, title: "PM-KISAN / Subsidy Schemes", desc: "Land ownership can be auto-matched against beneficiary lists to correctly target farm subsidies and prevent duplicate claims." },
  { icon: Gavel, tone: COLORS.saffron, title: "Courts & Dispute Resolution", desc: "Immutable audit trails give courts a single verified timeline of ownership, cutting down multi-year litigation." },
  { icon: Building2, tone: COLORS.gold, title: "Municipal & Property Tax", desc: "Digitised area and ownership data can auto-populate property tax assessments, reducing manual survey effort." },
  { icon: ShieldQuestion, tone: COLORS.red, title: "Insurance", desc: "Crop and property insurers can validate land holdings instantly instead of relying on manual field verification." },
  { icon: Network, tone: COLORS.navy, title: "DigiLocker / ULPIN", desc: "Records can sync with DigiLocker and the National ULPIN (Bhu-Aadhaar) registry for a single portable land identity." },
];
          ...
          <div className="flex items-center gap-3">
            <div className="hidden items-center gap-2 rounded-full bg-white/10 px-1 py-1 sm:flex" role="tablist">
              <button
                ...
                aria-selected={view === "officer"}
                onClick={() => setView("officer")}
                className={"rounded-full px-4 py-1.5 text-sm font-medium transition " + (view === "officer" ? "bg-white text-slate-900 shadow" : "text-white/70 hover:text-white")}
                style={view === "officer" ? { color: COLORS.navy } : {}}
              >
                <span className="inline-flex items-center gap-1.5"><ShieldCheck size={14} /> Officer HITL</span>
              </button>
              <button
                role="tab"
                aria-selected={view === "citizen"}
                onClick={() => { setView("citizen"); navigate("/citizenHome"); }}
                className="rounded-full px-4 py-1.5 text-sm font-medium transition text-white/70 hover:text-white hover:bg-white/10"
              >
                <span className="inline-flex items-center gap-1.5"><Users size={14} /> Citizen Portal</span>
              </button>
            </div>

            <button className="relative text-white/80 hover:text-white" aria-label="Notifications">
              <Bell size={18} />
              <span className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-red-500 text-[9px] flex items-center justify-center font-bold">3</span>
            </button>

            <button onClick={() => navigate("/login")} className="text-xs bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg border border-white/20 transition font-mono">
              Logout
            </button>
          </div>
        </div>
      ...
      <main>
        <section className="w-full p-4 sm:p-6 lg:p-10">
          <div className="mb-7 flex flex-col justify-between gap-5 md:flex-row md:items-end">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden rounded-full px-4 py-2 text-sm font-medium sm:flex items-center gap-2 shadow-sm border border-emerald-200" style={{ background: "#EAF6EE", color: COLORS.green }}>
                <span className="h-2.5 w-2.5 rounded-full animate-ping" style={{ background: COLORS.green }} />
                Model Engine Live
              </div>
              <button onClick={() => navigate("/upload")} className="flex w-fit items-center justify-center gap-2 rounded-xl px-5 py-3 font-semibold text-white shadow-lg transition hover:-translate-y-0.5" style={{ background: "linear-gradient(120deg, " + COLORS.green + ", " + COLORS.navy + ")" }}>
                <Upload size={18} /> Upload & Extract Document
              </button>
            </div>
          </div>
          ...
          <div className="mb-9 rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-white">
            <video
              ...
              src="/assets/videos/dashboard-hero.mp4"
              autoPlay
              muted
              loop
              playsInline
              poster="/assets/videos/dashboard-hero-poster.jpg"
            >
              {/* Fallback for browsers/old builds without the video file */}
              <HeroIllustration className="w-full h-auto block" />
            </video>
          </div>
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <ActionCard icon={Upload} title="AI Document Ingestion" description="Upload scanned Khasra/Khatauni sheets for instant multimodal OCR parsing." tone={COLORS.green} onClick={() => navigate("/upload")} />
              <ActionCard icon={CheckSquare} title="HITL Review Queue" description="Inspect low-confidence fields and authorize final land mutation records." tone={COLORS.navy} onClick={() => navigate("/review-queue")} />
              <ActionCard icon={Map} title="GIS Cadastral Map" description="Cross-verify parcel boundaries with digitized survey vector layers." tone={COLORS.saffron} onClick={() => navigate("/map")} />
            </div>
          </div>
          ...
          <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: COLORS.green }}>Beyond This Hackathon</p>
            <h3 className="mt-2 text-2xl font-bold" style={{ fontFamily: "'Merriweather', serif" }}>
              What this verified data can power next
            </h3>
            <p className="mt-3 max-w-3xl text-sm sm:text-base text-slate-600">
              Once a land record is digitised and officer-verified, it becomes a trusted data source other government
              departments and institutions can plug into — instead of every one of them re-verifying paper documents
              from scratch.
            </p>
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {INTEROP.map((item) => (
                <div key={item.title} className="rounded-xl border border-slate-200 p-4">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg mb-3" style={{ background: item.tone + "1A", color: item.tone }}>
                    <item.icon size={18} />
                  </div>
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs font-mono font-medium px-2 py-0.5 rounded" style={{ background: tone + "15", color: tone }}>Live</span>
      </div>
      <p className="text-2xl font-bold" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>{value}</p>
      <p className="text-sm font-medium text-slate-700 mt-1">{title}</p>
      <p className="text-xs text-slate-400">{hindi}</p>
      <p className="mt-3 text-xs font-semibold flex items-center gap-1" style={{ color: tone }}>{change}</p>
    </div>
  );
}
...
function ActionCard({ icon: Icon, title, description, tone, onClick }) {
  return (
    <button onClick={onClick} className="group flex w-full items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg focus:outline-none">
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-white shadow-md" style={{ background: tone }}>
        <Icon size={21} />
      </div>
      <div className="min-w-0">
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm h-full flex flex-col justify-between">
      <div>
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full font-mono font-medium">Sync Active</span>
        </div>
        <div className="divide-y divide-slate-100">
          {rows.map((r) => (
            <div key={r.id} className="flex items-center justify-between py-3.5">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ background: r.tone + "1A", color: r.tone }}>{r.status}</span>
              </div>
            </div>
          ))}
        </div>
      </div>
      <button onClick={() => navigate("/upload")} className="mt-5 w-full py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 transition">
        View Full Database Registry →
      </button>
    </div>
  );
